...
Your gift will support our programmes and those we serve.
...
""")
    st.markdown("---")
    st.markdown("#### Here’s a summary of your donation:")
    st.markdown(f"""
- **Amount**: {formatted_amount} ({'Monthly' if d['recurring'] else 'One-off'})  
- **Programme**: {d['appeal']}  
- **Payment method**: {d['payment']}  
- **Gift Aid**: {"Yes – we’ll claim an extra 25%" if d['gift_aid'] else "No"}  
- **Name**: {d['first_name']} {d['last_name']}  
- **Address**: {d['address']}  
- **Email**: {d['email']}
    """)
    st.info("If you need to make any changes, email us at **hello@campaignforfemaleducation.org**.")
    st.balloons()
